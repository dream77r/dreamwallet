          ...
          {deficitDay && (
            <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-800 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300">
              ...
              <div className="text-sm">
                ...
                {new Date(deficitDay.date).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}
                &nbsp;— ожидаемый баланс&nbsp;
                <span className="font-semibold text-red-600 dark:text-red-400">{formatAmount(deficitDay.balance)}</span>
              </div>
            </div>
          )}
